...
class Autoloader
{
    ...
    private function configureKint(): void
    {
        ...
        if (property_exists('Kint\Kint', 'depth_limit')) {
            Kint::$depth_limit = $config->maxDepth;
        } elseif (property_exists('Kint\Kint', 'max_depth')) {
            Kint::$max_depth = $config->maxDepth;
        }

        if (property_exists('Kint\Kint', 'display_called_from')) {
            Kint::$display_called_from = $config->displayCalledFrom;
        }

        if (property_exists('Kint\Kint', 'expanded')) {
            Kint::$expanded = $config->expanded;
        }

        if (isset($config->plugins) && is_array($config->plugins) && property_exists('Kint\Kint', 'plugins')) {
